...
- 单 source/scope 内容命令的 `index_unavailable.nextAction.commands[].command` 同样闭包 source/selector/db/json，并标记 `sideEffect:"write_index"`；兼容 `argv` 只作镜像。无显式 source 的跨 source find 仍保留默认 Codex bootstrap alternatives。
