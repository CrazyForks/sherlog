...
export async function collectCodexSourceSnapshot(selector: Selector, options: CollectSourceFilesOptions = {}): Promise<SourceSnapshot> {
  const canonical = canonicalizeSelector(selector);
  const allFiles = await collectCodexSourceFiles(canonical.root, {
    ...options,
